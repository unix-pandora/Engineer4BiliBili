import os
import sys
sys.path.append('./api/reading')
sys.path.append('./api/video_info')
sys.path.append('./api/entity')
sys.path.append('./api/command')
sys.path.append('./api/writing')
sys.path.append('./api/consts')
...
def assign(origin_path, output_path):
  ...
  # 编辑媒体文件的二进制内容
  modified_audio_name=edit_m4s_bytes(video_info_obj,origin_path, MineConstants.audio_name())
  video_info_obj.new_audio_path=modified_audio_name
  modified_video_name=edit_m4s_bytes(video_info_obj,origin_path, MineConstants.video_name())
  ...
  # 构建命令语句
  command_line = get_shell_line.build_command_line(final_file_path,video_info_obj,output_path) 
  # 执行命令
